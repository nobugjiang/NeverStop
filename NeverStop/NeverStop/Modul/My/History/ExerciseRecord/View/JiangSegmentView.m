...
@end
@implementation JiangSegmentView
...
- (void)btnClick:(UIButton *)btn{
    ...
    [UIView animateWithDuration:.2 animations:^{
        ...
        CGFloat lineW = [self.titlesStrWidthArray[btn.tag - 1051] floatValue];
...
- (void)updateselectLineFrameWithoffset:(CGFloat)offsetx{
    ...
    for (UIButton *btn in _btns) {
        ...
        if (btn.tag - 1051 == _defaultIndex - 1) {
            _titleBtn = btn;
            
            CGFloat w = [self.titlesStrWidthArray[btn.tag - 1051] floatValue];//字的宽度
            CGFloat line_x = (_btn_w - w) / 2 + (btn.tag - 1051) * _btn_w;
            CGFloat nextw = 0;
            if (offsetx < (btn.tag - 1051) * windowContentWidth) {
                nextw = [self.titlesStrWidthArray[btn.tag - 1052] floatValue];
                CGFloat max_w = (_btn_w * 2 + w + nextw) / 2;
                CGFloat endoffsetx = (btn.tag - 1051) * windowContentWidth - offsetx;
                ...
                nextw = [self.titlesStrWidthArray[btn.tag - 1051] floatValue];
                CGFloat max_w = (_btn_w * 2 + w + nextw) / 2;
                CGFloat endoffsetx = offsetx - (btn.tag - 1051) * windowContentWidth;
                if (_selectLine.layer.width < max_w) {
                    _selectLine.layer.width = w + MIN(endoffsetx, max_w);
                }else{
                    _selectLine.layer.width = max_w;
